...
def print_discordance(d, qid, ideal_queries, exp_queries, similarity_function, verbose=True):
    print(f'{(runid := runids[d.runid])} '
    ...
    exp_method = r[0]
    num_terms = int(r[1])
    exp_query_file = '/'.join([myglobals.expanded_query_path, exp_method, myglobals.coll, runid]) + ".term_weights"
    ...
    if response.lower() != "y" and response.lower() != "yes": return


def examine_discordances_for_query(paired_lists_dict, ideal_queries, exp_queries, similarity_function):
    while (qid := input("Query no.? (return to quit): ")):
        ...
        for d in paired_list[:top_how_many]:
            print_discordance(d, qid, ideal_queries, exp_queries, similarity_function, verbose=True)
        for d in paired_list[-top_how_many:]:
            print_discordance(d, qid, ideal_queries, exp_queries, similarity_function, verbose=True)
...
if __name__ == '__main__':
    if len(sys.argv) != 2 :
        sys.exit(f'Usage: {sys.argv[0]} <paired-lists file name>\n'
    ...
    examine_discordances_for_query(paired_lists_dict, ideal_queries_dict,
                                   exp_queries_dict, similarity_function)
